...
# Load libraries ####
library(here)        # Simplifies file path management across different directories
library(dplyr)       # Provides data manipulation with the pipe operator (%>%)
library(purrr)       # Implements functional programming tools for data operations
library(lubridate)   # Handles date and time operations, such as extracting weekdays
library(cosinor)     # Performs cosinor analysis for modeling periodic data
library(zoo)         # Includes functions for time series data, such as na.approx for imputation

# tell here where we are
here::i_am("repository/RScripts/Data_Preprocessing_and_Aggregation.R")
...
output_data_dir <- "repository/data/working_data"
...
# Save the data frame to the specified directory
save(bt_with_history_encoded, file = file.path(output_data_dir, "bt_with_history_hour_encoded.RData"))
